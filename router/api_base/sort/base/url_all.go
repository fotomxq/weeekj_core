...
func URLAll(routers *gin.RouterGroup, sortObj *ClassSort.Sort, permission string) {
	//获取分类列表
	...
	routers.PUT("", func(c *gin.Context) {
		...
		if err == nil {
			RouterUserRecord.CreateByC(c, "创建新的分类(", params.Name, ")[", data.ID, "]")
		}
		//反馈数据
	...
	routers.POST("/info", func(c *gin.Context) {
		...
		if err == nil {
			RouterUserRecord.CreateByC(c, "修改分类(", params.Name, ")[", params.ID, "]")
		}
		//反馈数据
	...
	routers.DELETE("/id", func(c *gin.Context) {
		...
		if err == nil {
			RouterUserRecord.CreateByC(c, "删除分类[", params.ID, "]")
		}
		//反馈数据
