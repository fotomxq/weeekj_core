...
func LoginAfter2(c *Router2Mid.RouterURLHeaderC, userInfo *UserCore.FieldsUserType, loginMark string, rememberMe bool, loginFrom string) {
	//用户是否被禁止登陆？
	if !checkUserNotBan(c, userInfo) {
		return
	}
	...
	type reportData struct {
		...
		UserName string `json:"userName"`
		// 用户详情
		UserData UserCore.FieldsUserType `json:"userData"`
	}
	//组装用户基本结构体
	newData := reportData{
		TokenID:        newTokenInfo.ID,
