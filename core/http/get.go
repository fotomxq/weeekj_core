...
// GetGoqueryStrInHtml 查询HTML中存在多少个字符串
// param sendURL string URL地址
// param findStr string 要搜索的字符串
// return int 存在多少个，-1为失败，0没找到
func GetGoqueryStrInHtml(doc *goquery.Document, findStr string) int {
	html, err := doc.Html()
	if err != nil {
		return -1
	}
...
func GetData(getURL string, params url.Values, proxyIP string, isSetHeader bool) ([]byte, error) {
	resp, err := GetResp(getURL, params, proxyIP, isSetHeader)
	if err != nil {
		return nil, err
	}
...
func GetResp(getURL string, params url.Values, proxyIP string, isSetHeader bool) (*http.Response, error) {
	//初始化参数
	...
	//如果需要对头信息加密，则进行加密处理
	if isSetHeader {
		req.Header.Add("User-Agent", GetUserAgentRand())
	}
	//执行URL获取
	...
	if status != 200 {
		return resp, errors.New(fmt.Sprint("status not 200, status code: ", status))
	}
	//defer resp.Body.Close()
	return resp, err
}

// GetGoquery 通过goquery获取HTML
// param getURL string 获取URL地址
// param params url.Values POST参数，如果留空则GET
// param proxyIP string 代理IP地址，如果留空跳过
// param isSetHeader bool 是否加入头信息加密，建议爬虫使用
// return *goquery.Document , error 文档操作句柄，是否成功
func GetGoquery(getURL string, params url.Values, proxyIP string, isSetHeader bool) (*goquery.Document, error) {
	resp, err := GetResp(getURL, params, proxyIP, isSetHeader)
	if err != nil {
		return nil, err
	}
...
// GetUserAgentRand 随机获取一个UserAgent头信息
// return string 伪造头信息
func GetUserAgentRand() string {
	var r = rand.New(rand.NewSource(CoreFilter.GetNowTime().UnixNano()))
	return HTTPGetUserAgents[r.Intn(len(HTTPGetUserAgents))]
}
