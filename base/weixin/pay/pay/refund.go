...
func (r Refunder) Refund(client *BaseWeixinPayClient.ClientType) (rres RefundedResponse, err error) {
	data, err := r.prepare(client.ConfigData.Key)
	if err != nil {
		return
	}
	resData, err := BaseWeixinPayClientUtil.TSLPostXML(client.BaseURL+"/secapi/pay/refund", data, client.ConfigData.CertDir+CoreFile.Sep+"apiclient_cert.pem", client.ConfigData.CertDir+CoreFile.Sep+"apiclient_key.pem")
	if err != nil {
		return
	}
...
func HandleRefundedNotify(res http.ResponseWriter, req *http.Request, key string, fuck func(RefundedNotify) (bool, string)) error {
	...
	key, err = BaseWeixinPayClientCrypto.MD5(key)
	if err != nil {
		return err
	}
	key = strings.ToLower(key)

	bts, err := BaseWeixinPayClientCrypto.AesECBDecrypt(ciphertext, []byte(key))
	if err != nil {
		return err
	}
