...
func (o *Order) prepare(key string) (order, error) {
	od := order{
		...
		TradeType: "JSAPI",
		SignType:  "MD5",
		NonceStr:  BaseWeixinPayClientUtil.RandomString(32),
	}
	...
	if o.IP == "" {
		ip, err := BaseWeixinPayClientUtil.FetchIP()
		if err != nil {
			return od, err
		}
	...
	sign, err := BaseWeixinPayClientCrypto.SignByMD5(signData, key)
	if err != nil {
		return od, err
	}
	od.Sign = sign
...
func GetParams(client *BaseWeixinPayClient.ClientType, nonceStr, prepayID string) (p Params, err error) {
	...
	p.Timestamp = strconv.FormatInt(CoreFilter.GetNowTime().Unix(), 10)
	p.SignType = "MD5"
	p.NonceStr = nonceStr
	p.Package = "prepay_id=" + prepayID
	p.PaySign, err = BaseWeixinPayClientCrypto.SignByMD5(map[string]string{
		"appId":     client.ConfigData.AppID,
		"signType":  p.SignType,
		"nonceStr":  nonceStr,
		"package":   p.Package,
		"timeStamp": p.Timestamp,
	}, client.ConfigData.Key)
	return
}
...
func (o Order) Unify(client *BaseWeixinPayClient.ClientType) (pres PaidResponse, err error) {
	reqData, err := o.prepare(client.ConfigData.Key)
	if err != nil {
		return
	}
	data, err := BaseWeixinPayClientUtil.PostXML(client.BaseURL+"/pay/unifiedorder", reqData)
	if err != nil {
		return
	}
...
func (q Query) Query(client *BaseWeixinPayClient.ClientType) (resp OrderResponse, err error) {
	q.SignType = "MD5"
	q.NonceStr = BaseWeixinPayClientUtil.RandomString(32)
	signData := map[string]string{
		"appid":     q.AppID,
		"mch_id":    q.MchID,
	...
	sign, err := BaseWeixinPayClientCrypto.SignByMD5(signData, client.ConfigData.Key)
	if err != nil {
		return
	}
	q.Sign = sign
	data, err := BaseWeixinPayClientUtil.PostXML(client.BaseURL+"/pay/orderquery", q)
	if err != nil {
		return
	}
