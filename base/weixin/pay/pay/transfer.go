...
func (t Transferer) Transfer(client *BaseWeixinPayClient.ClientType) (res TransferResponse, err error) {
	reqData, err := t.prepare(client.ConfigData.Key)
	if err != nil {
		return
	}
	resData, err := BaseWeixinPayClientUtil.TSLPostXML(client.BaseURL+transferAPI, reqData, client.ConfigData.CertDir+CoreFile.Sep+"apiclient_cert.pem", client.ConfigData.CertDir+CoreFile.Sep+"apiclient_key.pem")
	if err != nil {
		return
	}
