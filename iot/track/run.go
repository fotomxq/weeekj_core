...
func Run() {
	...
	defer func() {
		if r := recover(); r != nil {
			CoreLog.Error("iot track run, ", r)
		}
	}()
	time.Sleep(time.Second * 10)
	//启动定时器
	runTimer = cron.New()
	if err := runTimer.AddFunc("0 35 3 * * *", func() {
		if runHistoryLock {
			return
		}
		runHistoryLock = true
		//获取归档时间
		historyConfig, err := BaseConfig.GetDataString("IOTTrackHistoryTime")
		if err != nil {
			historyConfig = "-168h"
		}
		...
		if err = CoreSQLHistory.Run(&CoreSQLHistory.ArgsRun{
			...
			CoreLog.Error("iot track, ", err)
		}
		runHistoryLock = false
	}); err != nil {
		CoreLog.Error("iot track run, cron time, ", err)
	}
	runTimer.Start()
	//卡住进程
