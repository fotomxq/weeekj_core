...
func (t FieldsOtherOption) GetData() (DataOtherOptions, error) {
	var data DataOtherOptions
	data.Sort1 = DataOtherOptionSort{
		Name:    t.Sort1.Name,
		Options: t.Sort1.Options,
	}
	data.Sort2 = DataOtherOptionSort{
		Name:    t.Sort2.Name,
		Options: t.Sort2.Options,
	}
	for _, v := range t.DataList {
		data.DataList = append(data.DataList, DataOtherOptionChild{
			Sort1:         v.Sort1,
			Sort2:         v.Sort2,
			ProductID:     v.ProductID,
			Key:           v.Key,
			PriceReal:     v.PriceReal,
			PriceExpireAt: CoreFilter.GetISOByTime(v.PriceExpireAt),
			Price:         v.Price,
