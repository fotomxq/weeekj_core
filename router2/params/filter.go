...
func filterParamChild(data interface{}, checkMark string, _ string, isEmpty bool, min, max int) (errCode string, b bool) {
	//检查
	switch checkMark {
	case "id":
		//一个ID
	...
	case "desArr":
		...
		if isEmpty && len(val) < 1 {
			...
			for _, valV := range val {
				b = CoreFilter.CheckDes(valV, min, max)
				if !b {
					break
				}
			}
	...
	case "address_data":
		...
		if isEmpty && val.Address == "" && val.Phone == "" {
			b = true
		} else {
