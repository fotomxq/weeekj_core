...
// URLAll 组织管理层级操作接口汇总
func URLAll(r *Router2Mid.RouterURLOrg, appendURL string, tagObj *ClassTag.Tag, permissions []string, haveView bool) {
	if haveView {
		//获取标签列表
		r.POST(fmt.Sprint(appendURL, "/list"), func(c *Router2Mid.RouterURLOrgC) {
			//日志
			c.LogAppend = "om url class tag get list"
			//权限检查
		...
		r.POST(fmt.Sprint(appendURL, "/more"), func(c *Router2Mid.RouterURLOrgC) {
			//日志
			c.LogAppend = "om url class tag get more"
			//权限检查
			if !Router2Mid.CheckPermission(c, permissions) {
				return
			}
	...
	r.PUT(fmt.Sprint(appendURL, ""), func(c *Router2Mid.RouterURLOrgC) {
		//日志
		c.LogAppend = "om url class tag create tag"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
	...
	r.POST(fmt.Sprint(appendURL, "/info"), func(c *Router2Mid.RouterURLOrgC) {
		//日志
		c.LogAppend = "om url class tag update tag"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
	...
	r.DELETE(fmt.Sprint(appendURL, "/id"), func(c *Router2Mid.RouterURLOrgC) {
		//日志
		c.LogAppend = "om url class tag delete tag"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
