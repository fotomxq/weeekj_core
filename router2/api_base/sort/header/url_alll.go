...
// URLAll 有头层级的分类服务API
func URLAll(r *Router2Mid.RouterURLHeader, appendURL string, sortObj *ClassSort.Sort, permissions []string, haveView bool) {
	if haveView {
		//获取分类列表
	...
	//创建新的分类
	r.PUT(fmt.Sprint(appendURL, ""), func(c *Router2Mid.RouterURLHeaderC) {
		//日志
	...
	//修改分类
	r.POST(fmt.Sprint(appendURL, "/info"), func(c *Router2Mid.RouterURLHeaderC) {
		//日志
	...
	//删除分类
	r.DELETE(fmt.Sprint(appendURL, "/id"), func(c *Router2Mid.RouterURLHeaderC) {
		//日志
