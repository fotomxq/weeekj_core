package Router2APIBaseSortManager
...
// URLAll 管理层级操作接口汇总
func URLAll(r *Router2Mid.RouterURLUser, appendURL string, sortObj *ClassSort.Sort, permissions []string, haveView bool) {
	if haveView {
		//获取分类列表
		r.POST(fmt.Sprint(appendURL, "/list"), func(c *Router2Mid.RouterURLUserC) {
			//日志
			c.LogAppend = "manager url class sort get list"
			//权限检查
		...
		r.POST(fmt.Sprint(appendURL, "/more"), func(c *Router2Mid.RouterURLUserC) {
			//日志
			c.LogAppend = "manager url class sort get more"
			//权限检查
			if !Router2Mid.CheckPermission(c, permissions) {
				return
			}
		...
		//获取指定分类ID
		r.POST(fmt.Sprint(appendURL, "/id"), func(c *Router2Mid.RouterURLUserC) {
			//日志
			c.LogAppend = "manager url class sort get by id"
			//权限检查
			if !Router2Mid.CheckPermission(c, permissions) {
				return
			}
	...
	r.PUT(fmt.Sprint(appendURL, ""), func(c *Router2Mid.RouterURLUserC) {
		//日志
		c.LogAppend = "manager url class sort create sort"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
	...
	r.POST(fmt.Sprint(appendURL, "/info"), func(c *Router2Mid.RouterURLUserC) {
		//日志
		c.LogAppend = "manager url class sort update sort"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
	...
	r.DELETE(fmt.Sprint(appendURL, "/id"), func(c *Router2Mid.RouterURLUserC) {
		//日志
		c.LogAppend = "manager url class sort delete sort"
		//权限检查
		if !Router2Mid.CheckPermission(c, permissions) {
			return
		}
